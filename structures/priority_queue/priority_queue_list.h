...
namespace structures
{
	...
	inline PriorityQueueList<T>& PriorityQueueList<T>::operator=(const PriorityQueueList<T>& other)
	{
		if(this != &other)
		{
			...
			{
				;
				list_->add(new PriorityQueueItem<T>(item->getPriority(), item->accessData()));
			}
		}
		return *this;
	}
	...
	{
		for (PriorityQueueItem<T> *tmp : *list_)
		{
			delete tmp;
		}
		list_->clear();
	...
	inline int PriorityQueueList<T>::indexOfPeek() const
	{
		if(size() < 1)
		{
			throw std::exception("chyba, je prazdny");
		}
		else
		{
			int index;
			int maxpriorita = list_->operator[](0);//(*list_)[0]
			int maxpriorityindex = -1;
			for (PriorityQueueItem<T> *item : *list_)
			{
				if (maxpriorita > item->getPriority())
				{
