...
namespace structures {
	...
	private:
		...
		/// <summary> Iterator pre ArrayList. </summary> inerator je vnorena trieda, aby mal pristup k celej strukture a nemuselo sa porusit zapuzdrenie
	...
	{
		DSRoutines::rangeCheckExcept(index, size_, "Chyba!!");
		T pom = array_->operator[](index);
		if (index != size_ - 1) {
			Array<T>::copy(*array_, index, *array_, index - 1, size_);
		}
		size_--;
		return  pom;
	}
